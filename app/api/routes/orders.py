...
from app.models.commerce import Cart, CartItem, NotificationLog, Order, OrderItem, Payment
...
def _active_cart(db: Session, tenant_id: int, user_id: int):
    return db.query(Cart).filter(Cart.tenant_id == tenant_id, Cart.user_id == user_id, Cart.status == "ACTIVE").first()
...
def confirm_payment(payload: ConfirmPaymentRequest, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    order = db.query(Order).filter(Order.id == payload.order_id, Order.user_id == current_user.id, Order.tenant_id == current_user.tenant_id).first()
    if not order:
    ...
    db.commit()
    return {"message": "Payment confirmed", "order_id": int(order.id), "order_status": order.order_status}

@router.get("/orders/current")
def current_orders(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    rows = db.query(Order).filter(
        Order.user_id == current_user.id,
        Order.tenant_id == current_user.tenant_id,
        Order.order_status.in_(["PENDING", "PAID", "CONFIRMED", "PROCESSING", "SHIPPED"])
    ).order_by(Order.id.desc()).all()
    return [{"order_id": int(r.id), "order_number": r.order_number, "status": r.order_status, "total_amount": float(r.total_amount)} for r in rows]
...
def order_history(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    return [{"order_id": int(r.id), "order_number": r.order_number, "status": r.order_status, "payment_status": r.payment_status, "total_amount": float(r.total_amount)} for r in rows]
